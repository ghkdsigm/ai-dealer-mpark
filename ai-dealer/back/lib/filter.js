...
const BODYTYPE_RULES = [
    { re: /(스포티지|투싼|쏘렌토|싼타페|카니발|펠리세이드|모하비|QM6|QM5|XM3|티볼리|트레일블레이저|콜로라도|GV70|GV80|니로|셀토스|베뉴|스토닉|코란도|티구안|RAV4|CR-?V|CX-?5|SUV|밴|승합|픽업)/i, type: 'suv' },
    { re: /(아반떼|쏘나타|그랜저|K3|K5|K7|K8|K9|SM6|말리부|임팔라|제타|패사트|아테온|C\s*클래스|E\s*클래스|S\s*클래스|A[468]|3\s*시리즈|5\s*시리즈|7\s*시리즈|세단)/i, type: 'sedan' },
  ...
  export function filterAndRank(rawVehicles, intent = {}) {
    ...
    let filtered = vehicles.filter(v => {
      ...
      if (Array.isArray(options) && options.length) {
        ...
        const ok = options.some(opt =>
          list.some(x => String(x).toLowerCase().includes(String(opt).toLowerCase()))
        )
        if (!ok) return false
      }
    ...
    const hasHard =
      Number.isFinite(kmMin) || Number.isFinite(kmMax) ||
      Number.isFinite(yearMin) || Number.isFinite(yearMax) ||
      Number.isFinite(budgetMin) || Number.isFinite(budgetMax) ||
      Number.isFinite(monthlyMin) || Number.isFinite(monthlyMax) ||
      !!bodyType || !!fuelType ||
      (brands && brands.length) || (models && models.length) ||
      (colors && colors.length) || (options && options.length) ||
      (typeof noAccident === 'boolean') || (typeof shortKm === 'boolean')
    ...
    const scored = filtered.map(v => {
      ...
      if (Array.isArray(options) && options.length && Array.isArray(v.options)) {
        const hit = options.reduce(
          (acc, opt) => acc + (v.options.some(x => String(x).toLowerCase().includes(String(opt).toLowerCase())) ? 1 : 0),
          0
        )
