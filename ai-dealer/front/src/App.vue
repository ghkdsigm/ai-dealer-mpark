...
<template>
	<main class="min-h-screen bg-gray-50 p-6 max-w-5xl mx-auto">
		...
		<div class="flex flex-wrap items-center gap-2 mb-2">
			...
			<!-- 추천 실행 버튼(동의/요청 시 수동 실행) -->
			<button
				@click="recommend"
				class="px-4 py-2 rounded border"
				:disabled="loading"
				title="수집된 조건으로 최종 추천 실행"
			>
				추천 실행
			</button>
		...
		<div class="grid md:grid-cols-2 gap-4">
			...
			>
				...
				<p class="mt-1 text-sm">
					주행 <strong>{{ formatNumber(kmOf(v)) }}</strong> km
				</p>
...
/* 자동 전송 설정: STT가 최종 인식 문장을 내보낼 때 자동으로 chat() 실행 */
const autoSend = ref(true)

function echoQuery(s) {
	if (!s) return ''
	// 간단한 자연어 에코: 문장 끝 처리
...
/* 최종 추천 실행 (/api/recommend) */
async function recommend() {
	// 입력이 있으면 그걸 조건으로 사용하고, 없으면 빈 문자열 전송(백엔드가 세션/의도 사용 가능)
	const asked = (q.value || '').trim()
	loading.value = true
	error.value = ''
	try {
		const r = await fetch('/api/recommend', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ q: asked, limit: 5 })
		})
		if (!r.ok) throw new Error(`HTTP ${r.status}`)
		const data = await r.json()
		items.value = Array.isArray(data.items) ? data.items : []
		// reply 구성: 서버에서 reply 오면 우선 사용
		if (data.reply) {
			reply.value = data.reply
		} else if (items.value.length) {
			const top = items.value[0]
			reply.value = `${echoQuery(asked)} ${items.value.length}건을 찾았어요. 대표 추천: ${top.year ?? ''} ${top.make} ${top.model}.`
		} else {
			reply.value = `${echoQuery(asked)}는 현재 조건에 맞는 매물이 없어요. 범위를 조금 넓혀 다시 시도해 주세요.`
		}
		if (reply.value) speak(reply.value)
		// 추천 후 입력은 유지(사용자가 조건을 손보도록)
	} catch (e) {
		console.error(e)
		error.value = String(e.message ?? e)
	} finally {
...
function initSTT() {
	...
	recognition.onresult = e => {
		...
		if (finalText) {
			...
			/* 자동 전송: 최종 문장 들어오면 약간 디바운스해서 chat() 실행 */
			if (autoSend.value) {
...
/* 마이크 버튼 파동: title로 버튼을 식별하고 STT on 상태는 기존 클래스(bg-blue-600)로 감지 */
button[title='마이크로 질문 말하기'] {
	position: relative;
	overflow: visible;
}
button[title='마이크로 질문 말하기'].bg-blue-600 {
...
button[title='마이크로 질문 말하기'].bg-blue-600::before,
button[title='마이크로 질문 말하기'].bg-blue-600::after {
	content: '';
	position: absolute;
	inset: -8px;
